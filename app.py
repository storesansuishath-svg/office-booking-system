...
if choice == "📝 จองใหม่":
    ...
    with col2:
        ...
        # --- ส่วนแปลงค่า (Logic ฉลาดขึ้น: ถ้าลืมใส่ : ระบบจะใส่ให้เอง) ---
        try:
            # ถ้าพิมพ์เลข 4 ตัว (เช่น 0830) ให้เติม : กลาง (เป็น 08:30)
            if len(t_start_key) == 4 and ":" not in t_start_key:
                t_start_key = f"{t_start_key[:2]}:{t_start_key[2:]}"
            if len(t_end_key) == 4 and ":" not in t_end_key:
                t_end_key = f"{t_end_key[:2]}:{t_end_key[2:]}"
