...
def send_line_notification(booking_id, resource, name, dept, t_start, t_end, purpose, destination, status_text="ส่งคำขอใหม่"):
    ...
    payload = {
        ...
        "date": start_str,
        "end_date": end_str,
        "destination": destination, # ส่งค่าปลายทางไปด้วย
            ...
            for item in pending_items:
                with st.container(border=True):
                    ...
                    with col3:
                        ...
                        # ปุ่มอนุมัติ (แก้ไขให้ส่ง edit_dest)
                        if btn_app.button("✅", key=f"app_{item['id']}", use_container_width=True):
                            up_data = {"resource": edit_res, "requester": edit_req, "dept": edit_dept, "start_time": edit_start, "end_time": edit_end, "purpose": edit_purp, "destination": edit_dest, "status": "Approved"}
                        ...
                        if btn_rej.button("❌", key=f"rej_{item['id']}", use_container_width=True):
                            ...
                            st.rerun()
                        if btn_can.button("🗑️", key=f"can_{item['id']}", use_container_width=True):
        ...
        if not df.empty:
            ...
            with st.expander("คลิกเพื่อแก้ไขรายการ"):
                ...
                with st.form("edit_form_table"):
                    ...
                    # เพิ่มช่องแก้ปลายทางใน Form นี้ด้วย
                    ...
                    if b_save.form_submit_button("💾 บันทึก"):
                        if pw == "1234":
                            # เพิ่ม "destination": n_dest เข้าไปในคำสั่ง Update
                            supabase.table("bookings").update({
                                "resource": n_res, "requester": n_req, 
