...
if choice == "📝 จองใหม่":
    ...
    with col2:
        ...
        # 2. เวลาเริ่ม (ใส่ : รอไว้ให้เลย)
        t_start_key = st.text_input("ระบุเวลาเริ่ม (รูปแบบ 24 ชม.)", value="08:00", help="พิมพ์เป็น HH:mm เช่น 08:30")
        
        st.markdown("---")
        
        d_end = st.date_input("เลือกวันที่สิ้นสุด", value=d_start, min_value=d_start)
        # 3. เวลาสิ้นสุด (ใส่ : รอไว้ให้เลย)
        t_end_key = st.text_input("ระบุเวลาสิ้นสุด (รูปแบบ 24 ชม.)", value="17:00", help="พิมพ์เป็น HH:mm เช่น 17:30")
        
        reason = st.text_area("วัตถุประสงค์การใช้งาน")

        # --- ส่วนแปลงค่า (Logic เติม : อัตโนมัติ) ---
        try:
            # ถ้าพนักงานลบ : ทิ้ง หรือพิมพ์เลขติดกันมา 4 ตัว (เช่น 0900) ระบบจะใส่ให้เอง
            s_key = t_start_key.replace(":", "")
            if len(s_key) == 4: t_start_key = f"{s_key[:2]}:{s_key[2:]}"
            
            e_key = t_end_key.replace(":", "")
            if len(e_key) == 4: t_end_key = f"{e_key[:2]}:{e_key[2:]}"
