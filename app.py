...
def send_line_notification(booking_id, resource, name, dept, t_start, t_end, purpose, destination, status_text="Pending"):
    ...
    # แก้ไขเฉพาะส่วน payload ในฟังก์ชัน send_line_notification
    payload = {
        "id": booking_id,
        "resource": resource,
        "name": name,
        ...
        "end_date": end_str,
        "purpose": purpose,
        "destination": destination # << เพิ่มบรรทัดนี้ ข้อมูลถึงจะไปโชว์ใน LINE ครับ
    }
...
if choice == "📝 จองใหม่":
    ...
    with col1:
        ...
        if cat == "รถยนต์":
            ...
            # ส่วนของปุ่ม Google Maps (ต้องย่อหน้าให้ตรงกับ destination)
            if destination:
            ...
            # กรณีเลือก "ห้องประชุม"
            res = st.selectbox("เลือกห้อง", ["ห้องชั้น 1 (ห้องใหญ่)", "ห้องชั้น 2", "ห้อง VIP", "ห้องชั้นลอย", "ห้อง Production"])
            destination = "Office"

        # ข้อมูลผู้จอง (ดึงออกมาไว้นอก if-else เพื่อให้กรอกได้ทุกกรณี และย่อหน้าให้ตรงกับ cat)
