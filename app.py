...
# --- 2. ฟังก์ชันส่ง LINE Notification ---
...
# --- [หน้าตารางงาน + ระบบแก้ไข] ---
    ...
    with tab_edit:
        # --- เพิ่มหัวข้อการแก้ไข ---
        st.subheader("🛠️ แก้ไขข้อมูลรายการที่อนุมัติแล้ว")
        st.info("ระบุ ID ของรายการที่ต้องการแก้ไข และใส่รหัสผ่านเพื่อบันทึก")
        ...
        if not pending_res.data:
            ...
            for item in pending_res.data:
                with st.expander(f"📥 คำขอจากคุณ {item['requester']} - {item['resource']}"):
                    # --- เพิ่มหัวข้อการแก้ไขในหน้า Admin ---
                    st.markdown("#### ✏️ ตรวจสอบและแก้ไขรายละเอียดก่อนอนุมัติ")
